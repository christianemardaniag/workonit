...
@Component({
  selector: 'app-admin-about-workonit-edit',
  templateUrl: './admin-about-workonit-edit.component.html',
  styleUrls: ['./admin-about-workonit-edit.component.css']
})
export class AdminAboutWorkonitEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.systemContentObj = this.db.getAboutWorkOnIt();
    this.checkIfAuth();
  ...
  uploadImage(event: any) {
    ...
    this.dbstorage.uploadProfilePicture(event.target.files[0], `images/System Contents/AboutWorkOnItImage`).pipe(
      this.toast.observe({
        loading: "Image is being uploaded...",
        success: "Image uploaded!",
        error: "There was  an error uploading"
      }), concatMap((photoURL) => this.db.updateWorkOnItImage(photoURL))
    ).subscribe();
  }
  editAboutWorkOnItText(value: any) {
    this.db.editAboutWorkOnItText(value.text);
  }
